...
- **note 與每日 journal。** note 是你的, 一則一個檔。journal 每天早上由 `claude -p` 從
  那天的完整紀錄寫出來, 講的是那天的東西是怎麼做出來、怎麼運作的: 一個機制一節
  (一條流程、一個整合、一個修法), 名詞第一次出現就解釋, 步驟照系統運作的順序, 每一步
  對到檔案跟指令, 最後是怎麼確認它真的這樣跑。你再補自己的話, 截圖直接貼進去。兩者都存在
  你帳號底下的 Supabase, 本機有快取, 離線也看得到。編輯器是 HackMD 那套 Markdown:
  highlight、註腳、`$數學$` (KaTeX)、`:::spoiler` 折疊與 `:::info` 方框、`[TOC]`、
  程式碼上色加複製鈕, 預覽旁邊還有一欄目錄。
- **每個專案一張心智圖。** 每個專案最上方一塊 Excalidraw 畫布, 就是 Excalidraw 本體
  (官方套件打包進來, 不連外網), 你說存才存, 存成一份 `.excalidraw` 文件在同一個帳號底下。
