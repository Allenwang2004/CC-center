...
function collect(): Group[] {
  ...
  for (const cwd of Object.keys(store.report?.entries ?? {})) get(cwd);
...
function projectBlock(g: Group): HTMLElement {
  ...
  const block = sticky(store.openProjects, g.cwd,
    ...
    h("summary", { class: "project-head" },
      ...
      g.lastActive
        ? h("span", { class: "tag quiet", title: new Date(g.lastActive).toLocaleString() },
            `active ${ago((Date.now() - g.lastActive) / 1000)}`)
        : null,
      g.rows.length ? h("span", { class: "tag" }, `${asked(g.rows)} asked`) : null,
      // A ternary, not `&&`: a 0 would land in the heading as text.
      (added || removed) ? h("span", { class: "delta" },
        h("span", { class: "plus" }, `+${count(added)}`),
        h("span", { class: "minus" }, `−${count(removed)}`)) : null,
