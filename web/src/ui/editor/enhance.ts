...
export function enhance(preview: HTMLElement): Heading[] {
  /* code: a Copy button on every fence, colour when the library is in */
  const codes = [...preview.querySelectorAll<HTMLElement>("pre > code")];
  ...
  if (codes.length)
    void hljs().then((lib) => {
      for (const code of codes) if (code.isConnected) lib.highlightElement(code);
    }).catch(() => { /* the preview is still readable without colour */ });

  /* math: the span keeps its source in data-math until KaTeX replaces it */
  const maths = [...preview.querySelectorAll<HTMLElement>(".math[data-math]")];
  if (maths.length)
    void katex().then((lib) => {
      for (const el of maths) {
        if (!el.isConnected) continue;
        lib.render(el.dataset.math ?? "", el,
  ...
  for (const hd of preview.querySelectorAll<HTMLElement>(HEADINGS)) {
  ...
  preview.addEventListener("click", (e) => {
    ...
    const target = id && preview.querySelector<HTMLElement>(`#${CSS.escape(id)}`);
    if (!target) return;
    e.preventDefault();
...
export function outline(headings: Heading[], jump: (id: string) => void): HTMLElement {
