 ...
 */
export function wrap(text: string, sel: Sel, before: string, after = before): Edit {
  const { start, end } = sel;
 ...
 */
export function prefixLines(text: string, sel: Sel, prefix: string,
                            ordered = false): Edit {
  const from = lineStartOf(text, sel.start);
  const to = lineEndOf(text, sel.end);
  const lines = text.slice(from, to).split("\n");

  const has = (l: string): boolean =>
    ordered ? /^\s*\d+[.)]\s/.test(l) : l.startsWith(prefix);
  const strip = (l: string): string =>
    ordered ? l.replace(/^\s*\d+[.)]\s/, "") : l.slice(prefix.length);
...
export function heading(text: string, sel: Sel, level: number): Edit {
  const from = lineStartOf(text, sel.start);
  const to = lineEndOf(text, sel.end);
...
export function link(text: string, sel: Sel): Edit {
...
export function codeBlock(text: string, sel: Sel): Edit {
...
export function table(text: string, sel: Sel): Edit {
  const atLineStart = sel.start === lineStartOf(text, sel.start);
  const lead = atLineStart ? "" : "\n";
 ...
 */
export function footnote(text: string, sel: Sel): Edit {
  let n = 0;
...
export function spoiler(text: string, sel: Sel): Edit {
  const inner = text.slice(sel.start, sel.end);
  const atLineStart = sel.start === lineStartOf(text, sel.start);
  const lead = atLineStart ? "" : "\n";
...
export function indent(text: string, sel: Sel, out = false): Edit {
  const from = lineStartOf(text, sel.start);
  const to = lineEndOf(text, sel.end);
  const lines = text.slice(from, to).split("\n").map((l) =>
    out ? l.replace(/^ {1,2}/, "") : (l.trim() ? "  " + l : l));
  const body = lines.join("\n");
  return { text: text.slice(0, from) + body + text.slice(to),
           start: from, end: from + body.length };
}
