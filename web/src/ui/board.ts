...
import type { Excalidraw, ExcalidrawAPI, SceneJSON } from "../core/vendor.js";
...
export interface BoardOptions {
  cwd: string;
  host: string;
  /** The saved row's ref (the day it was first drawn), or "" when there is none yet. */
  ref: string;
  saved: string;
...
export interface Board {
  el: HTMLElement;
  /** New server data: adopted when the canvas has nothing unsaved. */
  sync(saved: string, ref: string, updatedAt: string | null): void;
}

interface Carrier extends HTMLDetailsElement {
  __board?: Board;
}
...
function parseScene(text: string): SceneJSON | null {
  if (!text.trim()) return null;
  try {
    const data = JSON.parse(text) as Partial<SceneJSON>;
...
export function board(options: BoardOptions): Board {
  const el = keep(`board:${options.cwd}`, () => build(options).el as Carrier) as Carrier;
  return el.__board!;
}

function build(options: BoardOptions): Board {
  let ref = options.ref;
  let saved = options.saved;
  let updatedAt = options.updatedAt;
  let status: Status = "clean";
  let inFlight = false;
  let lib: Excalidraw | null = null;
  let ex: ExcalidrawAPI | null = null;
  let mounted: { update(p: unknown): void; destroy(): void } | null = null;
  ...
  const root = sticky(store.openBoards, options.cwd, { class: "shelf mindmap board" },
    ...
    h("div", { class: "board-host" }, loading, canvas)) as Carrier;

  const look = (appState: Record<string, unknown>): string =>
    `${String(appState.viewBackgroundColor ?? "")}|${String(appState.gridSize ?? "")}`;
  ...
  async function mount(): Promise<void> {
    ...
    if (initial) savedLook = look(initial.appState);
    mounted = lib.mount(canvas, {
      ...
      excalidrawAPI: (handle) => {
        ex = handle;
        if (!initial) savedLook = look(handle.getAppState());
      },
      onChange: () => {
        if (inFlight || !ex) return;
        const next: Status = dirtyNow() ? "dirty" : (status === "saved" ? "saved" : "clean");
        if (next !== status) {
          status = next;
          paint();
        }
  ...
  root.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape" && root.classList.contains("is-expanded") && ex
      ...
      const st = ex.getAppState() as { selectedElementIds?: Record<string, boolean>;
                                       activeTool?: { type?: string }; editingTextElement?: unknown };
      const idle = !Object.keys(st.selectedElementIds ?? {}).length
        && (st.activeTool?.type ?? "selection") === "selection" && !st.editingTextElement;
      if (idle) expand.click();
    }
  }, true);
  ...
  const instance: Board = {
    ...
    sync(savedText, savedRef, savedAt) {
      ...
      if (savedText === saved && savedRef === ref) return;
      saved = savedText;
      ref = savedRef;
      updatedAt = savedAt;
      status = "clean";
