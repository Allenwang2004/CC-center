...
export function renderMachines(save: Save, refresh: (hosts?: string[]) => void): void {
  const host = document.getElementById("machines");
  if (!host || !store.report) return;
  ...
  const row = (
    ...
    return h("div", { class: `machine-row ${checked ? "" : "off"}`, data: { host: name } },
      ...
      isLocal
        ...
        : h("span", { class: "grip", role: "button", tabindex: "0" }, "∷"),
      box,
      h("span", { class: `pulse ${cls}` }),
      h("span", { class: "machine-name" }, name),
      isLocal ? h("span", { class: "tag" }, "here") : null,
      h("span", { class: "machine-count" },
        s?.status === "ok" ? String(s.n ?? 0) : s?.status === "error" ? "!" : ""),
      isLocal ? null : h("button", { class: "linkish danger", type: "button",
                                     data: { removeHost: name } }, "Remove"));
  };

  mount(host,
    row(local || "this machine", store.settings.include_local,
      (on) => void save({ include_local: on }), true),
    store.hosts.map((name) =>
      row(name, !off.has(name), (on) => {
...
export function renderStatus(connected: boolean): void {
  ...
  const last = store.report?.last_local
    ...
    : "not yet";
  mount(el,
    h("span", { class: `pulse ${connected ? "ok" : "bad"}` }),
    h("span", null, connected ? "Watching" : "Disconnected, retrying"),
    h("span", { class: "muted" }, `scanned ${last}`));
}

export function renderScope(): void {
  const el = document.getElementById("scope");
  if (!el || !store.report) return;
...
export function renderRemoteToggle(save: Save, refresh: () => void): void {
  const btn = document.getElementById("remote-toggle") as HTMLButtonElement | null;
  if (!btn) return;
...
export function bindMachineDrag(): void {
  const host = document.getElementById("machines");
  if (!host) return;
  ...
  host.addEventListener("pointerdown", (e) => {
    ...
    if (!grip) return;
    dragging = grip.closest<HTMLElement>(".machine-row");
    dragging?.classList.add("dragging");
    e.preventDefault();
  });
  host.addEventListener("pointermove", (e) => {
    if (!dragging) return;
    const rows = [...host.querySelectorAll<HTMLElement>(".machine-row")].filter(
      (r) => r.dataset.host !== store.localHost && r !== dragging);
  ...
  const drop = () => {
    ...
    dragging = null;
    const order = [...host.querySelectorAll<HTMLElement>(".machine-row")]
