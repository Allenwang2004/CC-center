...
function step(which) {
    ...
    fail(null);
    const focus = which === "email" ? $("signin-email")
        : $("signin-code");
    focus?.focus();
}
function setBusy(on) {
...
export function bindSignin(signedIn) {
    onSignedIn = signedIn;
    if (bound)
        return;
    bound = true;
...
export function showSignin(auth) {
    const screen = $("signin");
    const shell = $("shell");
    if (shell)
        shell.hidden = true;
    ...
    if (project)
        project.textContent = auth?.project ? `Supabase project ${auth.project}` : "";
