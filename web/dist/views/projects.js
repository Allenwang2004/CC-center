...
function collect() {
    ...
    for (const g of groups.values()) {
        ...
        for (const n of g.notes)
            g.lastActive = Math.max(g.lastActive, Date.parse(n.updated_at) || 0);
    }
    return [...groups.values()].sort((a, b) => b.lastActive - a.lastActive);
}
const matches = (turn, q) => !q ||
...
function noteShelf(g) {
    ...
    const select = (ref, focus = false) => {
        ...
        if (focus) {
            ...
            document.querySelector(`[data-editor="${CSS.escape(key)}"]`)
                ?.querySelector("input,textarea")?.focus();
        }
    };
    const ed = editor({
    ...
    const index = notes.map((n) => {
        ...
        const item = h("button", { class: `note-item${on ? " is-on" : ""}`, type: "button",
            aria: { current: on ? "true" : "false" } }, h("time", { class: "note-item-clock" }, refClock(n.ref)), h("span", { class: "note-item-title" }, firstLine(n.body, 60) || "Untitled"), h("span", { class: "note-item-date" }, n.ref.slice(5, 10)));
...
export function renderProjects(host) {
    ...
    if (badge)
        badge.textContent = groups.length ? String(groups.length) : "";
    if (!groups.length) {
...
function projectBlock(g) {
    ...
    const loose = g.loose.length
        ? h("section", { class: "day" }, h("header", { class: "day-head" }, h("h3", null, "Commits with no question behind them"), h("span", { class: "muted" }, "made by hand, or outside the agent")), h("div", { class: "ledger" }, [...g.loose]
            ...
            .map((c) => h("article", { class: "entry loose" }, h("time", { class: "margin-clock" }, (c.at ?? "").slice(5, 10)), h("div", { class: "entry-body" }, h("div", { class: "detail" }, h("span", { class: "detail-key" }, ""), h("span", { class: "detail-value" }, h("button", { class: "sha", type: "button", title: c.sha,
            data: { copy: c.sha } }, c.short), h("span", { class: "commit-subject" }, c.subject), h("span", { class: "muted" }, c.author))))))))
        : null;
