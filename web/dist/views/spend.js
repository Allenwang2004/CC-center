...
function readout(day, d) {
    if (!d)
        return h("p", { class: "spend-readout" }, h("strong", null, dayLabel(day)), h("span", { class: "muted" }, "nothing ran"));
    const models = Object.entries(d.models).sort((a, b) => b[1] - a[1]);
    const hosts = Object.entries(d.hosts).filter(([host]) => host !== store.localHost);
    return h("p", { class: "spend-readout" }, h("strong", null, dayLabel(day)), h("span", { class: "spend-cost" }, money(d.cost)), h("span", { class: "muted" }, `${count(d.calls)} calls`), h("span", { class: "muted" }, `in ${count(d.in)} · out ${count(d.out)} · cache read ${count(d.cache_read)} · cache write ${count(d.cache_write)}`), models.length > 1
        ? h("span", { class: "muted" }, models.map(([m, c]) => `${m} ${money(c)}`).join(", "))
        : models[0] ? h("span", { class: "muted" }, models[0][0]) : null, hosts.length
        ? h("span", { class: "muted" }, hosts.map(([host, c]) => `${host} ${money(c)}`).join(", "))
        : null);
}
function table(days, daily) {
    const cell = (text, cls = "") => h("td", { class: cls }, text);
    return h("details", { class: "spend-table" }, h("summary", null, `Every day, ${days.length} of them`), h("div", { class: "table-scroll" }, h("table", null, h("thead", null, h("tr", null, h("th", null, "Day"), h("th", { class: "num" }, "Calls"), h("th", { class: "num" }, "Input"), h("th", { class: "num" }, "Output"), h("th", { class: "num" }, "Cache read"), h("th", { class: "num" }, "Cache write"), h("th", { class: "num" }, "Cost"))), h("tbody", null, [...days].reverse().map((day) => {
        const d = daily[day];
        return h("tr", null, cell(dayLabel(day)), cell(count(d.calls), "num"), cell(count(d.in), "num"), cell(count(d.out), "num"), cell(count(d.cache_read), "num"), cell(count(d.cache_write), "num"), cell(money(d.cost), "num cost"));
    })))));
}
export function renderSpend() {
    ...
    const show = (day) => detail.replaceChildren(readout(day, daily[day]));
    show(latest);
    ...
    const cells = ["Mon", "", "Wed", "", "Fri", "", ""].map((t, i) => h("span", { class: "spend-day", style: { gridColumn: "1", gridRow: String(i + 1) } }, t));
    const months = [];
    let lastMonth = -1;
    for (let w = 0; w < weeks; w++) {
        for (let i = 0; i < 7; i++) {
            ...
            if (i === 0 && d.getMonth() !== lastMonth && (w === 0 || d.getDate() <= 7)) {
                ...
                months.push(h("span", { class: "spend-month", style: { gridColumn: String(w + 2) } }, MONTH[d.getMonth()]));
            }
            if (future) {
    ...
    return h("section", { class: "block spend" }, h("h4", null, "Spend", h("span", { class: "muted" }, `at API list prices · every day on record, whatever the range · since ${dayLabel(days[0])}`)), h("p", { class: "spend-summary" }, h("strong", { class: "spend-cost" }, money(total)), h("span", { class: "muted" }, ` across ${days.length} days · ${count(totalTokens)} tokens`
        + ` · busiest ${dayLabel(busiest)} at ${money(daily[busiest].cost)}`)), h("div", { class: "spend-calendar" }, h("div", { class: "spend-months", style: { gridTemplateColumns: columns } }, months), grid, h("div", { class: "spend-legend" }, h("span", { class: "muted" }, "less"), [0, 1, 2, 3, 4].map((l) => h("span", { class: `spend-cell l${l}` })), h("span", { class: "muted" }, "more"))), detail, table(days, daily));
}
