...
const serverNow = () => (store.report ? store.report.server_now : Date.now() / 1000);
const secondsSince = (iso) => serverNow() - epoch(iso);
/* -- meters ------------------------------------------------------------- */
/** Green while there is room, amber from 60%, red from 85%. The number says the same. */
const grade = (pct) => (pct >= 85 ? "high" : pct >= 60 ? "mid" : "");
function meter(pct, cls = "") {
    const p = pct === null ? 0 : Math.max(0, Math.min(100, pct));
    return h("span", { class: `meter ${cls} ${grade(p)}`, role: "meter",
        aria: { valuenow: String(Math.round(p)), valuemin: "0", valuemax: "100" } }, h("span", { class: "meter-fill", style: { width: `${p}%` } }));
}
...
function ctxMeter(s) {
    ...
    return h("span", { class: "ctx",
        title: `${count(c.used)} of ${count(c.size)} tokens in context`
            + (c.model ? ` · ${c.model}` : "") }, h("span", { class: "muted" }, "ctx"), meter(pct), h("span", { class: "ctx-pct" }, `${Math.round(pct)}%`));
}
function resetLabel(w) {
    ...
    if (left <= 0)
        return `reset ${ago(-left)}`;
    return left < 86400 ? `resets in ${duration(left)}` : `resets ${whenLabel(w.resets_at)}`;
}
function planRow(name, w) {
    if (!w)
        return h("div", { class: "plan-row muted" }, h("span", { class: "plan-name" }, name), h("span", null, "not reported"));
    const expired = Boolean(w.resets_at && w.resets_at <= serverNow());
    const cls = expired ? "stale" : grade(w.used_percentage);
    return h("div", { class: `plan-row ${cls}` }, h("span", { class: "plan-name" }, name), meter(expired ? null : w.used_percentage), h("span", { class: "plan-pct" }, expired ? "—" : `${Math.round(w.used_percentage)}%`), h("span", { class: "muted" }, resetLabel(w)));
}
/**
...
function planBlock() {
    ...
    if (!u) {
        ...
        return h("section", { class: "block calm" }, h("h4", null, "Plan"), h("p", { class: "calm-line" }, "Claude Code reports the 5-hour and 7-day limits only to its status line. Route it through ", h("code", null, hook), " (add ", h("code", null, "-- <your current command>"), " to keep the one you have) and they show up here."));
    }
    const rl = u.rate_limits ?? {};
    const remote = u.host && u.host !== store.localHost ? ` · from ${u.host}` : "";
    return h("section", { class: "block" }, h("h4", null, "Plan", h("span", { class: "muted" }, `${u.model ? `${u.model} · ` : ""}as of ${ago(serverNow() - u.at)}${remote}`)), h("div", { class: "plan" }, planRow("5-hour", rl.five_hour), planRow("7-day", rl.seven_day)));
}
/* -- sessions ----------------------------------------------------------- */
...
function attentionCard(s) {
    const a = s.attention;
    return h("article", { class: "alert", id: `agent-${s.session_id}` }, h("div", { class: "alert-mark" }), h("div", { class: "alert-body" }, h("h3", null, a.label, a.tool && h("span", { class: "tag" }, a.tool)), h("p", { class: "alert-title" }, s.title), h("div", { class: "alert-meta" }, h("code", null, basename(s.cwd)), h("span", { class: "muted" }, s.host), h("span", { class: "muted" }, `stopped ${ago(secondsSince(a.since))}`)), ctxMeter(s), h("div", { class: "row-actions" }, h("button", { class: "btn", type: "button", data: { copy: resumeCommand(s) } }, "Copy resume command"), h("button", { class: "btn ghost", type: "button", data: { focusSession: s.session_id } }, "Open record"), controls(s))));
}
/*
...
function controls(s) {
    ...
    const interrupt = h("button", { class: "btn ghost tiny", type: "button",
        title: "Send Ctrl+C: stop what it is doing now, keep the conversation" }, "Interrupt");
    const end = h("button", { class: "btn ghost tiny danger", type: "button",
        title: "End the claude process; the session can be resumed later" }, "End");
...
function runningRow(s) {
    const tail = s.tail;
    const label = tail?.state === "tool"
        ? `running ${tail.tool ?? "a tool"}`
        : tail?.state === "waiting" ? "idle, last spoke" : "working";
    return h("article", { class: "run", id: `agent-${s.session_id}` }, h("span", { class: `pulse ${s.alive === true ? "on" : "unknown"}`,
        title: s.alive === true ? "A claude process is running in this folder"
            : "Could not check this machine for a claude process" }), h("div", { class: "run-body" }, h("p", { class: "run-title" }, s.title), h("div", { class: "run-meta" }, h("code", null, basename(s.cwd)), h("span", { class: "muted" }, s.host), h("span", { class: "muted" }, `${label} · ${ago(secondsSince(tail?.at))}`)), ctxMeter(s)), controls(s), h("button", { class: "btn ghost", type: "button", data: { copy: resumeCommand(s) } }, "Resume"));
}
export function renderAgents(host) {
    ...
    if (badge)
        badge.textContent = needing.length ? String(needing.length) : "";
    document.title = needing.length ? `(${needing.length}) cc-center` : "cc-center";
    const machines = h("section", { class: "block" }, h("h4", null, "Machines"), h("div", { class: "machine-grid" }, report.hosts.map((state) => {
        const disabled = (store.settings.disabled_hosts ?? []).includes(state.host);
        const cls = state.busy ? "busy" : state.status === "ok" ? "ok" : "bad";
        return h("div", { class: `machine ${disabled ? "off" : ""}` }, h("span", { class: `pulse ${cls}` }), h("span", { class: "machine-name" }, state.host), state.local && h("span", { class: "tag" }, "here"), h("span", { class: "muted" }, state.status === "ok"
            ? `${state.n ?? 0} sessions · ${state.ms ?? 0} ms`
            : state.error || "unreachable"), state.procs !== null && state.procs !== undefined
            ? h("span", { class: "muted" }, `${state.procs} claude running`)
            : h("span", { class: "muted" }, "process state unknown"));
    })));
    mount(host, needing.length
        ? h("section", { class: "block" }, h("h4", null, "Waiting on you"), needing.map(attentionCard))
        : h("section", { class: "block calm" }, h("h4", null, "Waiting on you"), h("p", { class: "calm-line" }, "Nothing is stopped. Every agent is either working or finished.")), planBlock(), live.length
        ? h("section", { class: "block" }, h("h4", null, "Working now", h("span", { class: "muted" }, `active in the last ${duration(window)}`)), live.map(runningRow))
        : null, machines);
}
export async function refreshNow(hosts) {
