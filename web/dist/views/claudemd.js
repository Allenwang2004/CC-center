...
const keyOf = (f) => `claudemd:${f.host}:${f.cwd ?? "~"}`;
export function setClaudeQuery(q) {
    query = q.trim().toLowerCase();
}
/** Read the list from disk again and redraw. */
export async function loadClaudeFiles(host) {
    loading = true;
    error = null;
    renderClaude(host);
    try {
...
function card(f) {
    ...
    const head = h("header", { class: "claude-head" }, h("h3", null, name), remote ? h("span", { class: "tag remote" }, f.host) : h("span", { class: "tag" }, "this machine"), f.exists ? null : h("span", { class: "tag quiet" }, "not created yet"), f.error ? h("span", { class: "tag gone" }, f.error) : null, h("code", { class: "project-path", title: f.path }, f.path));
    let body;
    if (open) {
        const ed = editor({
            key,
        ...
        const create = h("button", { class: "btn ghost", type: "button" }, isGlobal ? "Create ~/.claude/CLAUDE.md" : "Create .claude/CLAUDE.md");
        create.addEventListener("click", () => {
            creating.add(key);
            const pane = document.getElementById("pane-claude-body");
            if (pane)
                renderClaude(pane);
            document.querySelector(`[data-editor="${CSS.escape(key)}"] textarea`)?.focus();
        });
        body = h("div", { class: "claude-create" }, h("p", { class: "muted" }, f.error ? `Could not read ${f.path}.` : "No file here yet. Claude Code reads it if it exists."), f.error ? null : create);
    }
...
export function renderClaude(host) {
    ...
    const projects = all.filter((f) => f.scope === "project" && (!query || basename(f.cwd).toLowerCase().includes(query)
    ...
    mount(host, h("section", { class: "claude-group" }, h("h2", null, "Global", h("span", { class: "muted" }, "~/.claude/CLAUDE.md on each machine, read before every project")), ...globals.map(card)), h("section", { class: "claude-group" }, h("h2", null, "Projects", h("span", { class: "muted" }, ".claude/CLAUDE.md inside the project folder")), projects.length
        ? projects.map(card)
        : h("div", { class: "empty" }, h("h3", null, query ? "No project matches" : "No projects"), h("p", null, query ? "Try another name." : "Run Claude Code somewhere and it will show up here."))));
}
